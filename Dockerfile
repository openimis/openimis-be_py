...
RUN apt-get update && apt-get install -y apt-transport-https ca-certificates gettext && apt-get upgrade -y
...
RUN test "$DB_ENGINE" != "django.db.backends.postgresql" && ACCEPT_EULA=Y apt-get install -y msodbcsql17 mssql-tools unixodbc-dev || :
