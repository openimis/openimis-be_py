...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if 'opensearch_reports' in apps.app_configs:
            ...
            self.upload_opensearch_configuration()
    ...
    def upload_opensearch_configuration(self):
        """
        ...
        host_domain = os.getenv('HOST_DOMAIN', 'http://localhost:8080')
        opensearch_endpoint = f'{host_domain}/opensearch/api/saved_objects/_import?overwrite=true'
        self.__upload_data(host_domain, opensearch_endpoint)

    def __upload_data(self, host_domain, opensearch_endpoint):
        base_path = Path(settings.BASE_DIR)
